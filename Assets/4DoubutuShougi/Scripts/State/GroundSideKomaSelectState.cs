...
public class GroundSideKomaSelectState : IShougiState
{
    ...
    public bool OnSquareSelected((int x, int y) pos, DoubutuShougiMap map)
    {
        ...
        if (map.map[(int)pos.x, (int)pos.y].playSide == DoubutuShougiPlaySideType.Ground)
        {
            komaPos = pos;
    ...
    {
        GroundSideDstSelectState state = new GroundSideDstSelectState(komaPos, komaType);
